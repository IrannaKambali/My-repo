...
public class Lib implements IAutoConstant {
	...
	public static String getCellValue(String sheetName,int rowNum,int colNum) {
		...
		try {
			FileInputStream exclObj=new FileInputStream(EXCEL_PATH);
			wb = WorkbookFactory.create(exclObj);
	...
	public static void capatureScreenshot(WebDriver driver,String testMethodName) {
		String currDateAndTime =new Date().toString().replaceAll(":","_");
		
		try {
			TakesScreenshot ts=(TakesScreenshot) driver;
			File srcFile=ts.getScreenshotAs(OutputType.FILE);
			File desFile=new File(SCREENSHOT_PATH + testMethodName+"__"+currDateAndTime +".png");
